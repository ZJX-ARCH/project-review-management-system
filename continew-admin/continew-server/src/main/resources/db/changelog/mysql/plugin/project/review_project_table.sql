...
CREATE TABLE IF NOT EXISTS `review_project` (
    ...
    `status`                 int           NOT NULL                    COMMENT '当前状态（对应 ProjectStatus 枚举 Integer value，如1=草稿/10=已提交/20=审核中）',
    `current_node_type`      varchar(20)   DEFAULT NULL                COMMENT '当前节点类型（AUDIT/REVIEW/DECISION/MANAGEMENT/ACCEPTANCE）',
